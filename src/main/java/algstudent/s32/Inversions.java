...
//must have a O(nlogn)-> for that the divide and conquer scheme must be a=b^k, using divide and conquer by division.
	...
	public String start() {
		...
		return String.valueOf(countInversions(0, ranking.size() - 1, this.ranking, 0));
	}
	...
	private long countInversions(int l, int r, List<Integer> ranking, int inversions) {
		...
		if (r > l) {
			int middle = (l + r) / 2;
			inversions +=countInversions(l, middle, ranking, inversions);
			inversions +=countInversions(middle + 1, r, ranking, inversions);// a=2. k=1
	...
	private long combineAll(int x1, int x2, int y1, int y2, List<Integer> ranking) {
		...
		while (!allEvaluated) {
			...
			// while there are values left to explore
			if (x[xi] > y[yi]) {// if the value of xi is greater than yi, INVERSION
				inversions++;
...
//				inversions++;// if a value of y is smaller than the value of x , an inversion is produced
//				if (yi == sizeY - 1) {
